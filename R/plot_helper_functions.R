...
get_color_scale <- function(values, aesthetics = "color", max_char_label = 15) {
  ...
  if (n_values == 0) {
    ...
    colors <- RColorBrewer::brewer.pal(n = n_values, name = "Dark2")
  } else {
    colors <- terrain.colors(n = n_values)
  }
...
#' @param fill_var A string. Variable name to set color.
...
get_uncertainty_geom <- function(data, bar_line_specs, fill_var = NULL, low_pattern = "CILow", high_pattern = "CIHigh") {
